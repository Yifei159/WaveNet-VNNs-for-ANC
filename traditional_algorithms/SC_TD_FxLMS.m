function [e, W] = SC_TD_FxLMS(x, d, secpath, con_len, normalized, mu, delta, eta)
%% Single-Channel-Time-Domain FxLMS Algorithm 
...
for n = 1:N
   ...
   if(length(x)*pert/100<n)
    ...
    e(n) = d(n) + s(n);
    %=================feedforward update====================
    feedforward_filtered_ref = secpath'*filter_ref_buffer;% Calculate filtered ref
    filtered_ref_buffer = [feedforward_filtered_ref; filtered_ref_buffer(1:con_len-1)];
    %feedforward con_filter update
    if normalized == 0                  
        W = W - mu*e(n)*filtered_ref_buffer; 
    end
    if normalized == 1           
        nfactor_f = filtered_ref_buffer.'*filtered_ref_buffer;  
        normalized_miu_f = mu/(nfactor_f + delta); 
        W = W - normalized_miu_f*e(n)*filtered_ref_buffer;
    end
 end 
end